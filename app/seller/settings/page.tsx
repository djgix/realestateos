  ...
  function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
    return (
      <label className="relative inline-flex items-center cursor-pointer">
        ...
        <div className="w-10 h-5 bg-slate-700 rounded-full peer peer-checked:after:translate-x-5 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-seller" />
      </label>
    )
  }
        ...
        <div className="lg:col-span-3">
          ...
          {tab === 'notifications' && (
            <div className="card p-6">
              ...
              <div className="space-y-4">
                {[
                  { key: 'new_offer', label: 'New offer received', desc: 'Get notified when a buyer submits an offer' },
                  { key: 'showing_request', label: 'Showing request', desc: 'Get notified when a buyer requests a showing' },
                  { key: 'price_change', label: 'Price change alerts', desc: 'Get notified when comparable listings change price' },
                  { key: 'offer_status', label: 'Offer status updates', desc: 'Get notified when an offer status changes' },
                ].map(item => (
                ...
                <button onClick={saveNotifications} disabled={savingNotif} className="btn-seller">
          ...
          {tab === 'billing' && (
            <div className="card p-6">
              ...
              <div className="mb-6 p-4 bg-seller/10 border border-seller/20 rounded-xl text-center">
                <p className="text-xs text-slate-500 mb-1">Current plan</p>
                <p className="font-display text-2xl text-seller capitalize">{profile?.plan || 'trial'}</p>
              </div>
              {profile?.plan !== 'paid' && (
                <div>
                  <p className="text-slate-400 text-sm mb-4">Unlock the full SellerOS platform to manage your listing end-to-end.</p>
                  <button onClick={() => checkout('seller')} className="btn-seller">
